...
{
    public partial class pv_mise : Form
    {
        public pv_mise()
        {
            InitializeComponent();
        ...
        {
            cdb.con.Open();
            cdb.cmd = new SqlCommand("select nom , mat_agent   from identification where num=" + cmb_num.SelectedItem, cdb.con);
            SqlDataReader dr = cdb.cmd.ExecuteReader();
            while (dr.Read())
            {
                txt_nom.Text = dr[0].ToString();
                txt_mat.Text = dr[1].ToString();
            }
            cdb.con.Close();

            cdb.con.Open();
            cdb.cmd = new SqlCommand("select Servicee  from DEMANDE where Num_DEMANDE=" + cmb_num.SelectedItem, cdb.con);
            dr = cdb.cmd.ExecuteReader();
            while (dr.Read())
            {
                txt_service.Text = dr[0].ToString();
            }
            cdb.con.Close();
        }
        ...
        private void pv_mise_Load(object sender, EventArgs e)
        {
